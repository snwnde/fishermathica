...
volumeEff[zv_,kv_]:=((ndensEffective[zv,kv,0.1]*powerSpectrum[zv,kv])/(1+ndensEffective[zv,kv,0.1]*powerSpectrum[zv,kv]))^2
...
veffs= volumeEff[zv, kv]*volumeSurvey[zv, $zbinGlobal];
pobs=powerSpectrum[zv,kv];
cov = (pref/veffs)*pobs^2 * (1/kv)^3;
Return[cov]
]


covMatGC[0.5,1.0, 0.99]
